...
async function findPath(map, start, end) {
    ...
    async function insert([x, y], pre, formStart) {
        if (map[100 * y + x] !== 0) {
            return;
        }
        ...
        if(formStart >= tables[100*y+x])
            return ;

        map[100 * y + x] = pre

        tables[100*y+x] = formStart
    ...
    // while (queue.length) {
        ...
        await insert([x - 1, y], [x, y], formStart + 1)
        await insert([x + 1, y], [x, y], formStart + 1)
        await insert([x, y - 1], [x, y], formStart + 1)
        await insert([x, y + 1], [x, y], formStart + 1)

        await insert([x - 1, y - 1], [x, y], formStart + 1.4)
        await insert([x + 1, y - 1], [x, y], formStart + 1.4)
        await insert([x - 1, y + 1], [x, y], formStart + 1.4)
        await insert([x + 1, y + 1], [x, y], formStart + 1.4)
    }
